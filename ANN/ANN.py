...
def getIms(testLb, predLb):
    for i in range(len(testLb)):
        realLabl = getLabel(testLb[i])
        predictLabl = getLabel(predLb[i])
        # Save a mispredicted image if labels dont match
        if (saved[realLabl] == 0 and realLabl != predictLabl):
            saveIm(testImag[i], "Real" + str(realLabl) + "Predict" + str(predictLabl) + ".png")
            saved[realLabl] = 1
...
model.add(Dense(500, input_shape=(28*28, ), kernel_initializer='he_normal'))  # first layer
model.add(Activation('relu'))  # Non linear Activation

# 1

model.add(Dense(300, input_shape=(500, ), kernel_initializer='he_normal'))  # first layer
model.add(Activation('relu'))  # Non linear Activation

# 2
model.add(Dense(100, input_shape=(300, ), kernel_initializer='he_normal'))
model.add(Activation('relu'))  # Non linear Activation

# 4
model.add(Dense(50, input_shape=(100, ), kernel_initializer='he_normal'))
model.add(Activation('relu'))  # Non linear Activation
...
# Train Model
history = model.fit(trainingImag,
                    trainingLabl,
                    validation_data=(validationImag, validationLabl),
                    epochs=200,
                    batch_size=1024)
...
getIms(testLabl, predicted_lable)

print(np.argmax(testLabl, axis=1).shape)

matrix = confusion_matrix(tl, pl)
...
print(metrics.classification_report(tl, pl))
